...
export function useUser() {
  return useContext(UserContext) || {
    ...
    legalCase: null, caseReminders: [], subscription: null,
    ...
    loadSubscription() {}, refreshTokenBalance() {},
  };
}
...
export function UserProvider({ children }) {
  ...
  const loadUserData = useCallback(async (userId) => {
    ...
    // Load subscription
    await loadSubscriptionInner();
  }, []);

  async function loadSubscriptionInner() {
    try {
  ...
  function refreshTokenBalance(tokenInfo) {
    ...
    setSubscription(prev => {
      ...
      return {
        ...prev,
        remaining: tokenInfo.remaining != null ? tokenInfo.remaining : prev.remaining,
        plan_id: tokenInfo.plan || prev.plan_id,
      };
    });
  }
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        ...
        // Validate session — if token is expired, treat as logged out
        if (u && session?.expires_at && session.expires_at * 1000 < Date.now()) {
  ...
  async function signInWithGoogle() {
    // Clear stale localStorage token so OAuth redirect always fires
    const storageKey = `sb-${new URL(process.env.NEXT_PUBLIC_SUPABASE_URL).hostname.split('.')[0]}-auth-token`;
    try { localStorage.removeItem(storageKey); } catch {}
  ...
  return (
    <UserContext.Provider value={{
      ...
      chatSessions, userMemory, legalCase, caseReminders, subscription,
