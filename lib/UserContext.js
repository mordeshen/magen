...
export function UserProvider({ children }) {
  ...
  // מונע re-entrance של loadUserData — כאשר onAuthStateChange יורה כמה events ברצף
  const loadingFor = useRef(null);

  const loadUserData = useCallback(async (userId) => {
    // אם כבר טוענים את אותו user — דלג
    if (loadingFor.current === userId) return;
    loadingFor.current = userId;
    try {
      await loadUserDataInner(userId);
    } finally {
      if (loadingFor.current === userId) loadingFor.current = null;
    }
  }, []);

  const loadUserDataInner = useCallback(async (userId) => {
    ...
    const { data: lc, error: lcErr } = await supabase
      ...
      .eq("user_id", userId)
      .maybeSingle();
    // AbortError קורה כש-auth refresh לוקח את ה-lock באמצע — לא שגיאה אמיתית, נתעלם
    if (lcErr && lcErr.name !== "AbortError") {
      console.warn("[user-context] legal_case error:", lcErr.message);
    }
    if (lc) {
      setLegalCase(lc);
  ...
  async function deleteAccount() {
    ...
    try {
      ...
      if (r.ok) {
        await supabase.auth.signOut();
        setUser(null); setProfile(null); setUserRights({});
        setShowProfilePanel(false); setLegalCase(null);
        setCaseReminders([]); setChatSessions([]); setUserMemory([]);
        setSubscription(null);
        if (typeof window !== "undefined") window.location.reload();
      } else {
        alert("שגיאה במחיקה — פנה אלינו לסיוע.");
      }
    } catch {
  ...
  return (
    <UserContext.Provider value={{
      ...
      saveSession, loadSession, deleteSession,
      saveMemory, clearMemory, clearAllSessions, deleteAccount,
      saveLegalCase, dismissReminder,
