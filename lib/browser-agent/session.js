...
class BrowserAgentSession {
  ...
  async captureAnonymizedScreenshot() {
    await this.page.evaluate(() => {
      document.querySelectorAll("input, textarea").forEach((el) => {
        el.dataset._origValue = el.value;
    ...
    await this.page.evaluate(() => {
      document.querySelectorAll("input, textarea").forEach((el) => {
        if (el.dataset._origValue !== undefined) {
          el.value = el.dataset._origValue;
          delete el.dataset._origValue;
        }
      });
    });
    return screenshot;
